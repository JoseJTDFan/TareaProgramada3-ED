...
ArbolAdministradores::ArbolAdministradores() {
	this->raiz = NULL;
}

bool ArbolAdministradores::arbol_Vacio() {
	return this->raiz == NULL;
}

puntero_Administradores ArbolAdministradores::getRaiz() {
	return this->raiz;
}

void ArbolAdministradores::buscar_Dato(int& pCedula, puntero_Administradores& pRaiz, bool& pEsta, int& pIndice_Rama)
{
...
void ArbolAdministradores::meter_Dato(int& pCedulaAux, string& pNombreAux, puntero_Administradores& pPaginaXr, puntero_Administradores& pRaiz, int& pIndice_Rama)
{
...
void ArbolAdministradores::empujar_Arriba(int& pCedula, string& pNombre, puntero_Administradores& pRaiz, bool& pEmpujar_Arriba, int& pCedula_Aux, string& pNombre_Aux, puntero_Administradores& pPaginaXr)
{
	int indice_Rama = 0;
	bool esta = false;
	if (pRaiz == NULL) {
		...
		if (esta)
			return;
		puntero_Administradores rama = pRaiz->getRama(indice_Rama);
		...
		if (pEmpujar_Arriba) {
			if (pRaiz->getCuentas() < 4) {
				...
				puntero_Administradores paginaAux = pPaginaXr;
				...
				int cedulaAuxD = 0;
				string nombreAuxD = "";
				puntero_Administradores paginaAux = NULL;
...
void ArbolAdministradores::inserta(int pCedula, string pNombre)
{
	...
	int cedula_Aux = 0;
	string nombre_Aux = "";
	puntero_Administradores paginaXr = NULL;
	puntero_Administradores paginaAux = NULL;

	this->empujar_Arriba(pCedula, pNombre, this->raiz, empujar_Arriba, cedula_Aux, nombre_Aux, paginaXr);

	if (empujar_Arriba) {
		paginaAux = new Pagina_Administradores(cedula_Aux, nombre_Aux);
...
void ArbolAdministradores::dividir_Pagina(int& pCedula_Aux, string& pNombre_Aux, puntero_Administradores& pPaginaXr, puntero_Administradores& pRaiz, int& pIndice_Rama, int& pCedula_Aux2, string& pNombre_Aux2, puntero_Administradores& pPaginaXr2)
{
	int posMda = 0;
	if (pIndice_Rama <= 2)
		posMda = 2;
		...
		posMda = 3;
	pPaginaXr2 = new Pagina_Administradores();
	puntero_Administradores rama = NULL;
...
void ArbolAdministradores::imprimir_Arbol()
{
	if (this->arbol_Vacio()) {
		cout << "\n\tArbolAdministradores vacio.\n";
		return;
	}
	this->raiz->imprimir();
}

string ArbolAdministradores::reporteAdministradores(){
    if (raiz == NULL) {
        return "ArbolAdministradores vacio";
    }
    
    return raiz->imprimirReporte();
}

void ArbolAdministradores::quitar(puntero_Administradores& pRaiz, int& pIndice_Rama)
{
...
void ArbolAdministradores::sucesor(puntero_Administradores& pRaiz, int& pIndice_Rama)
{
	puntero_Administradores pagina_Q = pRaiz->getRama(pIndice_Rama);
	int indice_Cero = 0;
...
void ArbolAdministradores::eliminar_Registro(int& pCedula, puntero_Administradores& pRaiz, bool& pEncontrado)
{
	int indice_Rama = 0;
	if (pRaiz == NULL)
		pEncontrado = false;
...
void ArbolAdministradores::eliminar(int pCedula)
{
	bool encontrado = false;
	puntero_Administradores pagina_P = NULL;
...
void ArbolAdministradores::mover_Derecha(puntero_Administradores& pRaiz, int& pIndice_Rama)
{
	puntero_Administradores rama_Trabajo = pRaiz->getRama(pIndice_Rama);
...
void ArbolAdministradores::mover_Izquierda(puntero_Administradores& pRaiz, int& pIndice_Rama)
{
	int indice = pIndice_Rama - 1;
	puntero_Administradores rama_Trabajo = pRaiz->getRama(indice);
...
void ArbolAdministradores::combinar(puntero_Administradores& pRaiz, int& pIndice_Rama)
{
	int indice = pIndice_Rama - 1;
	puntero_Administradores pagina_Q = pRaiz->getRama(pIndice_Rama);
	puntero_Administradores rama_Trabajo = pRaiz->getRama(indice);
...
void  ArbolAdministradores::restablecer(puntero_Administradores& pRaiz, int& pIndice_Rama)
{
	//cout<<"\t-------\n\tIndc: "<<pIndice_Rama<<"\n\t-------";
	if (pIndice_Rama > 0) {
		int indice = pIndice_Rama - 1;
...
bool ArbolAdministradores::esta_Administradores(int pCedula)
{
	if (this->raiz == NULL)
		return false;
...
void ArbolAdministradores::agregar_Datos_lectura(string& pDatosLinea)
{
	std::string datos[2] = { "", "" };
	int indiceDatos = 0;
